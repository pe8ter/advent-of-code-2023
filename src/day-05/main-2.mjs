...
/**
 * @typedef {{start: number, length: number}} SeedRange
 ...
 * @returns {{seeds: Array<SeedRange>, maps: RangeMap, connections: ConnectionMap}}
 */
...
function convertInputToData(inputText) {
    ...
    for (let i = 0; i < seedRegexResult.length; i += 2) {
        const start = Number.parseInt(seedRegexResult[i]);
        const length = Number.parseInt(seedRegexResult[i + 1]);
        const seedRange = { start, length };
...
async function main() {
    ...
    for (const seedRange of seeds) {
        for (let i = seedRange.start; i < seedRange.start + seedRange.length; ++i) {
            const location = findLocation(i, 'seed', maps, connections);
