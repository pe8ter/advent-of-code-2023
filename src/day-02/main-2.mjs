...
/**
 * @typedef {{red: number, green: number, blue: number}} CubeSet
 */

/**
 * @typedef {{index: number, sets: Array<CubeSet>}} Game
 */

/**
 * Extract the results for a "game" for an input line. A game includes its index and a list of collection of colors.
 * Each color represents the number of each cube color drawn in a set. Each set is separated by semi-colons. Each color
 ...
 * @returns {Game} Data structure that represent a game.
 */
...
/**
 * Find the minimum number of each color of cubes that a game must have.
 * @param {Game} game
 * @returns {CubeSet}
 */

function findMinimumNumberOfCubes(game) {
    const red = Math.max(...game.sets.map(set => set.red));
    const green = Math.max(...game.sets.map(set => set.green));
    const blue = Math.max(...game.sets.map(set => set.blue));
...
async function main() {
    ...
    const total = inputText
        ...
        .map((game) => findMinimumNumberOfCubes(game))
        .reduce((accum, min) => accum + min.red * min.green * min.blue, 0);
